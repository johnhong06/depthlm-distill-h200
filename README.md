...
| 작업 | 실행 명령어 | GPU | 비고 |
|---|---|---|---|
| 스모크 | `bash run.sh smoke` | 1 | 모델 다운로드 → 30 스텝 → 3 px 평가. 10 분 |
| 격자: 혼합 풀, 분포(KL) | `bash run.sh grid mixed soft` | 7 (또는 1) | 7 이면 셀 8개를 동시에 학습 |
| 격자: 혼합 풀, greedy(CE) | `bash run.sh grid mixed hard` | 7 (또는 1) | |
| 라벨링: 실내 풀 | `bash run.sh label indoor hf_xxxxxxxx` | **7** | 교사 30 GB × 4 병렬. 읽기 전용 토큰, 끝나면 폐기 |
| 라벨링: 실외 풀 | `bash run.sh label outdoor hf_xxxxxxxx` | **7** | |
| 격자: 실내·실외 × soft·hard (4개) | `bash run.sh grid indoor soft` 등 | 7 (또는 1) | 라벨을 저장소에 넣은 뒤 |

- 신청 창 안에서 이슈를 순서대로 낸다. 컨테이너는 끝나면 삭제되고 `/app/output/` 만 남는다(관리자에게 파일 요청). 리포트는 65,000자까지만 오므로 표준 출력은 요약, 전체 로그는 `/app/output/` 에 쓴다. 결과 폴더는 `results_<cond>_<pool>.zip` 으로도 묶인다.
